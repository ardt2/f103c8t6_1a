...
//            TRISE |= 8U;     // 200 нсек / 28 нсек ≈ 7. (7 + 1). Reset value 0x20, можно и просто TRISE = 8U;
        ...
        void WriteAddr(uint8_t addr)
        {
            ...
            DR = addr | 0x00U; // Адрес устройства. ( | 0x01U) для чтения.
            // Для более новых STM32 - << 1U, где адрес пишется в регистр конфигурации.
            while( (SR1 & I2C_SR1_ADDR) == 0 ) ; // Сброс флага ADDR
            (void)SR2;                            // чтением 2-х регистров состояния.
//
            CR1 |= I2C_CR1_STOP;
            while( (CR1 & I2C_CR1_STOP) != 0 ) ;
        }
        // --------------------------------------------------------------------
        ...
        // --------------------------------------------------------------------
        void Write(uint8_t addr, uint8_t data, uint8_t data2)
        {
            CR1 |= I2C_CR1_START; // Старт.
        ...
        // --------------------------------------------------------------------
        void FullReadOne(uint8_t addr, uint8_t reg, __IO uint8_t & data)
        {
            // Запись номера регистра для чтения.
            CR1 |= I2C_CR1_START; // Старт.
            while ((SR1 & I2C_SR1_SB) == 0) ;
            ...
            DR = reg;
            while( (SR1 & I2C_SR1_BTF) == 0 ) ; // Может быть не нужно, в принципе,
                                                // похоже, состояние буфферируется.
            CR1 |= I2C_CR1_STOP;
            while( (CR1 & I2C_CR1_STOP) != 0 ) ;
            ...
            CR1 |= I2C_CR1_POS; // NACK после первого прочитанного байта.
            ...
            CR1 |= I2C_CR1_STOP; // Стоп после первого прочитанного байта.

            while( (SR1 & I2C_SR1_RXNE) == 0 ) ;
            data = DR;
        }
        // --------------------------------------------------------------------
        void FullReadTwo(uint8_t addr, uint8_t reg, __IO uint8_t & data, __IO uint8_t & data2)
        {
            // Этап 1 - запись номера регистра для чтения.
            CR1 |= I2C_CR1_START; // Старт.
            while ((SR1 & I2C_SR1_SB) == 0) ;

            DR = addr; // | 0x01U - ниже, для чтения.
            while( (SR1 & I2C_SR1_ADDR) == 0 ) ;
            (void)SR2;
            ...
            CR1 |= I2C_CR1_ACK; // Подтверждение приема первого байта.

            DR = addr | 0x01U;  // ACK после следующего за адресом байта.

            while( (SR1 & I2C_SR1_ADDR) == 0 ) ;
            (void)SR2;


            CR1 &= ~I2C_CR1_ACK; // NACK после следующего за этим байта.
            CR1 |= I2C_CR1_POS;

            while( (SR1 & I2C_SR1_RXNE) == 0 ) ;
            data = DR;

            CR1 |= I2C_CR1_STOP; // Стоп после этого байта.
        ...
        // --------------------------------------------------------------------
        void FullReadN(uint8_t addr, uint8_t reg, __IO uint8_t * data, uint8_t n)
        {
            // Этап 1 - запись номера регистра для чтения.
            CR1 |= I2C_CR1_START; // Старт.
            while ((SR1 & I2C_SR1_SB) == 0) ;
            ...
            // Милый GCC, прости, сам разверну цикл.
            CR1 &= ~I2C_CR1_ACK; // NACK после следующего за этим байта.
            CR1 |= I2C_CR1_POS;
...
/*
        // --------------------------------------------------------------------
        void Read(uint8_t addr, uint8_t & data, uint8_t & data2)
        {
            CR1 |= I2C_CR1_START; // Старт.
            while ((I2C1->SR1 & I2C_SR1_SB) == 0) ;
